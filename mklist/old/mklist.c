...
static long lBigN= 0L ;
static long lLittleN= 0L ;
...
static void syntax()
{
	static void *pText[]= {
		...
		"Special substitution names:",
		"\t%@%\t\tname of item",
		"\t%*%\t\tlist of all keywords defined for item",
		"\t%x%\t\tvalue of keyword 'x' for item",
	} ;
...
static void doOutput(struct db_Row *pdbr, long lSortValue)
{
	/*
	...
	*/
	...
	outputAddItem(lSortValue, szBuff) ;
}
...
static DB_RC processEntry(struct db_Row *pdbr, DBRS_RC dbrsrc)
{
	...
	*/
	pevrc= evalEntry(&pexp_select, &lResult) ;
...
#endif
	if (!lResult) return(DBRC_SUCCESS) ;
	lLittleN++ ;			/* count item passing main test	*/

	/*
		evaluate sort expression
	*/
	pevrc= evalEntry(&pexp_sort, &lResult) ;
	if (pevrc != PEEVALRC_SUCCESS) return(DBRC_FAILURE) ;
	if (mlp.bReverseSort) lResult= -lResult ;

	doOutput(pdbr, lResult) ;
	return(DBRC_SUCCESS) ;
}
