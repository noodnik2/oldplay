...
/*
	int clperror((char *)msg);

	does "perror()" on message, exmplaining current error in 'errno'.

	warning: the command name must be shorter than 32 characters.
	warning: the total message size must be shorter than 132 characters.
*/
void clperror(msg)
char *msg;
{
	...
	sprintf(errmsg, "%s: %s", clcmdname(cmdname, clpname), msg);
	perror(errmsg) ;
}
