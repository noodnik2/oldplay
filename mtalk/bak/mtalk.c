...
static MTALK_RC doTk(MTALK_SOCK mtSock) ;
...
static struct mtalk_caller *pCreateCallerStruc(MTALK_SOCK mtSock) ;
...
{
	setsigcatch(signo) ;
	fprintf(stderr, "mtalk: signal caught: signo=%d\n", signo) ;
}
...
static MTALK_RC handleSpecialCmd(
	...
	if (strcmp(pszCLine, "/exit") == 0) {
		return(exitCaller(ciTalker, MTALKROLE_SENDER)) ;
	}
...
static void notifyOfExitingCaller(struct mtalk_caller *pCallerExiting)
{
	MTALK_ROLE mtRoleListener ;
	MTALK_CALLERIDX ciListener ;
	static szToWrite[MTALKMAX_CLINESIZE+MTALKMAX_CALLERNAMELEN+17] ;
...
static MTALK_RC exitCaller(MTALK_CALLERIDX ciTalker, MTALK_ROLE mtRole)
{
	struct mtalk_caller *pCallerTalker ;

	pCallerTalker= mtp.pmtca[ciTalker] ;
	if (pCallerTalker->mtState == MTALKSTATE_CHAT) {
		notifyOfExitingCaller(pCallerTalker, mtRole) ;
	}
	return(deleteCaller(ciTalker)) ;
}
...
static MTALK_RC exitCaller(MTALK_CALLERIDX ciTalker, MTALK_ROLE mtRole)
{
	...
	if (pCallerTalker->mtState == MTALKSTATE_CHAT) {
		...
		for (
			ciListener= 0;
			ciListener< MTALKMAX_NUMCALLERS;
			ciListener++
		) {
			pCallerListener= mtp.pmtca[ciListener] ;
			if (!pCallerListener) continue ;
		...
		writeToCaller(pCallerTalker, "Byebye!\n") ;
	}

	return(deleteCaller(pCallerTalker)) ;
}
...
static MTALK_RC muxCallerLine(MTALK_CALLERIDX ciTalker)
{
	...
	if (mtp.iDebugLevel> 0) {
		printf("%s says: '%s'\n", pCallerTalker->szName, pszCLine) ;
	}

	if (handleSpecialCmd(ciTalker, pszCLine, &bDone) != MTALKRC_SUCCESS) {
		return(MTALKRC_FAILURE) ;
	}
...
static MTALK_RC doTk(MTALK_SOCK mtSock)
{
	...
	pCaller= pCreateCallerStruc(mtSock) ;
	if (!pCaller) {
...
static MTALK_RC notifyRecipient(struct mtalk_caller *pCallerSender)
{
	...
	if (mtp.iDebugLevel> 1) {
		printf("notifying recipient of call from '%s'\n", pszCallerName) ;
	}
	...
	if (pCallerRecipient) {
		...
		if (mtp.iDebugLevel> 1) {
			printf(
				"%s joined to existing recipient: %s\n",
...
static MTALK_RC writeRawByteToCaller(struct mtalk_caller *pCaller, int iByte)
{
	...
	cByte= (char) iByte ;
	i= write(pCaller->mtSock, &cByte, 1) ;
...
#define		MTRB_GETBYTE(ch)					\
	...
	while(/*CONSTCOND*/1) {
		...
		MTRB_GETBYTE(ch2) ;
		if (mtp.iDebugLevel> 2) printf("nvt: %d %d\n", ch, ch2) ;
		if (ch2 == NVT_IAC) {
		...
		if (
			...
			MTRB_GETBYTE(ch3) ;
			if (mtp.iDebugLevel> 2) printf("nvto: %d\n", ch3) ;
		}
	}
