...
void main(int argc, char *argv[])
{
	init(argc, argv) ;
#if 0
	if (getPipeSock() != MTALKRC_SUCCESS) {
		exit(1) ;
	}
#endif
...
static void dumpstatus()
{
	...
	if (bInUse) {
		fprintf(stderr, "dumpstatus: recusion ignored\n") ;
		return ;
	}

	fp= fopen("mtalk.stat", "w") ;
	if (!fp) {
		fprintf(stderr, "dumpstatus: can't open\n") ;
		return ;
	}
...
static MTALK_RC getPipeSock()
{
	...
	fdin= open("mtalk.pi", O_RDONLY) ;
	if (fdin> 0) {
		fdout= open("mtalk.po", O_WRONLY) ;
		if (fdout> 0) {
			printf("using pipes %d %d\n", fdin, fdout) ;
			return(doTk(fdin, fdout, MTALKBOOL_FALSE)) ;
		}
		perror("no output pipe") ;
		close(fdin) ;
	}
...
static MTALK_RC exitCaller(MTALK_CALLERIDX ciExiter)
{
	struct mtalk_caller *pCallerExiter ;
	struct mtalk_caller *pCallerListener ;
	MTALK_ROLE mtRoleListener ;
	MTALK_CALLERIDX ciListener ;
	static char szToWrite[MTALKMAX_CLINESIZE+MTALKMAX_CALLERNAMELEN+17] ;

	pCallerExiter= mtp.pmtca[ciExiter] ;

	if (pCallerExiter->mtState == MTALKSTATE_CHAT) {
		mtRoleListener= (
			((pCallerExiter->mtRole) == MTALKROLE_RECIPIENT)
			?	MTALKROLE_SENDER
			:	MTALKROLE_RECIPIENT
		) ;
		sprintf(
		...
		writeToCaller(pCallerExiter, "Byebye!\n") ;
	}

	return(deleteCaller(pCallerExiter)) ;
}
#endif
