...
char *hostname ;
{
...
#if 1
		sock_in_p->sin_addr.s_addr= inet ;
#else
		sock_in_p->sin_addr.s_addr= htonl(inet) ;	/* wrong */
#endif
		return(0) ;
	}
	...
	FARmemcpy(
		(char FAR *)&sock_in_p->sin_addr,
		(char FAR *)hent->h_addr,
		hent->h_length
	) ;
	return(0) ;
}
...
struct sockaddr_in *addr_p ;
{
	int i ;
	char FAR *s ;
	extern char FAR *inet_ntoa() ;
	static char buf[65] ;

#if 1
	s= inet_ntoa((*(struct in_addr *)(&addr_p->sin_addr))) ;
	for (i= 0; s[i] != '\0'; i++) {
		buf[i]= s[i] ;
	}
